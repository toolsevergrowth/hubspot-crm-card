export default function handler(req, res) {
  try {
    console.log("Received query:", req.query); // log incoming params

    const egAccIdRaw = req.query.eg_acc_id;
    const egAccId = typeof egAccIdRaw === 'string' ? egAccIdRaw.trim() : '';

    const isValid = /^A[0-9a-zA-Z]{20,}$/.test(egAccId);
    ...
    res.status(500).send("Internal server error in embed function.");
  }
}
